...
* Desktop GUI
* Lokale applicatie
* Moet op de achtergrond kunnen draaien
* System tray
* Start with Windows
* Geen verplichte cloud/backend
...
Filesystem watcher + periodieke fallback is gewenst.

Bestanden die nog worden geschreven mogen niet onmiddellijk verwerkt worden.

Ondersteun bijvoorbeeld:

```text
Process only files unchanged for X seconds
```
...
Applicatie moet zonder hoofdvenster kunnen functioneren.
...
Normale gebruikers moeten deze complexiteit niet nodig hebben.
...
Het eindresultaat moet kunnen worden geleverd als normale Windows-applicatie.
...
| Filesystem watcher         | Zeer hoog |
| Background operation       | Zeer hoog |
| System tray                |      Hoog |
| Windows notifications      |      Hoog |
| Performance                |      Hoog |
| Reliability                | Zeer hoog |
| Async/concurrency          |      Hoog |
| Installer/distributie      |      Hoog |
